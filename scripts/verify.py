...
def need_file(r: Report, path: pathlib.Path, label: str, how: str) -> pathlib.Path | None:
    ...
    if path.stat().st_size == 0:
        ...
        return None
    if path.suffix == ".md" and UNANSWERED.search(path.read_text(encoding="utf-8", errors="ignore")):
...
def any_file(r: Report, patterns: list[str], label: str, how: str) -> bool:
    ...
    if not hits:
        ...
        return False
    stale = [p for p in hits if p.suffix == ".md" and UNANSWERED.search(p.read_text(encoding="utf-8", errors="ignore"))]
